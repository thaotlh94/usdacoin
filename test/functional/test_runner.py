...
def main():
    ...
    tmpdir = "%s/usdacoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and usdacoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "usdacoind"]) is not None:
            print("%sWARNING!%s There is already a usdacoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "USDACOIND" not in os.environ:
        os.environ["USDACOIND"] = build_dir + '/src/usdacoind' + exeext
        os.environ["USDACOINCLI"] = build_dir + '/src/usdacoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `usdacoin-cli help` (`rpc_interface.txt`).
